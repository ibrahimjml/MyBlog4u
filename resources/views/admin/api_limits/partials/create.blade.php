<div id="LimitModel" class="hidden fixed inset-0 z-30 bg-black bg-opacity-50 flex items-center justify-center p-4">
  ...
  <div class="bg-white rounded-lg shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col">
    ...
    <div class="flex-1 justify-start">
        <h2 class="text-xl font-bold text-gray-800">Create New Limit</h2>
    ...
    <div class="p-6 overflow-y-auto">
      <form id="addLimit" action="{{route('admin.api-limits.store')}}" method="POST" class="space-y-6">
        @csrf
        <!-- Limit Name -->
        <div>
        ...
        <div>
         ...
         <div class="p-2 rounded-lg border border-gray-300 w-fit flex items-center gap-4">
           ...
           <x-toggle name="status" value="{{ \App\Enums\ApiLimits\ApiLimitStatus::ACTIVE->value }}" :checked="old('status') ===  '{{\App\Enums\ApiLimits\ApiLimitStatus::ACTIVE->value}} '"/>
...
<script>
  ...
  const addLimitForm = document.getElementById('addLimit');
  const submitLimit = document.getElementById('submitLimit');

  function clearLimitErrors() {
    addLimitForm.querySelectorAll('[data-error-for]').forEach((element) => {
      element.textContent = '';
      element.classList.add('hidden');
    });
  }

  function showLimitErrors(errors) {
    Object.entries(errors).forEach(([field, messages]) => {
      const errorElement = addLimitForm.querySelector(`[data-error-for="${field}"]`);
  ...
  addLimitForm.addEventListener('submit', async (event) => {
    ...
    submitLimit.textContent = 'Creating...';

    try {
      const response = await fetch(addLimitForm.action, {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
          'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
        },
        body: new FormData(addLimitForm),
      });
      ...
      if (!response.ok) {
        if (response.status === 422 && data.errors) {
          showLimitErrors(data.errors);
          return;
        }

        throw new Error(data.message || 'Unable to create New Limit.');
      }
      ...
      addLimitForm.reset();
      document.getElementById('LimitModel').classList.add('hidden');
      ...
      submitLimit.textContent = 'Create';
    }
  });
</script>
@endpush
